...
export default function LogForm({ logs, clientName }: LogFormProps) {
  ...
  const formatLogTitle = (log: Record<string, any>) => {
    ...
    return `${user?.email} - ${formattedDate}`;
  };
  ...
  return (
    <div className="w-full">
      ...
      <div className="space-y-2">
        ...
        {log((log) => {
          const title = formatLogTitle(log);
          const uniqueKey = `${log.creator}-${log.created_at}`;
